...
Il faut donc un language moins puissant, assez expressif pour la programmation et assez {\em peu} expressif pour avoir un implémantation efficace.
Nous allons dans ce chapitre voir comment on obtient cette language.
...
Il n'y a pas moyen de contourner cette limite.
...
Pour pallier à ce problème on va faire trois choses:
\begin{itemize}
...
Typiquement, on utilisera des clauses de Horn, c'est à dire des clauses $C_i$ ne comportant qu'un littéral sans négation:
...
\item Choisir une clause spéciale, la clause résolvante, qui sera toujours utilisée dans la résolution.
Il n'y aura plus alors qu'a choisir une seule clause au lieu de deux pour chaque résolution.
\end{itemize}

Le langage Prolog utilise ces trois techniques pour obtenir un langage efficace.
...
un compromis très intéressant entre efficacité et expressivité.
...
Un programme en Prolog est un ensemble de clauses de Horn (règles; axiomes):
...
\begin{itemize}
\item On commence par mettre le but $G$ que l’on veut prouver dans $r$ (on ne note pas la négation).
...
Ainsi, l'algorithme trouvera plusieurs solutions si la reqûete $G$ a plusieurs solutions avec le programme $P$.
...
d'être récursif terminal, parce que l'exécution dépend aussi d'une pile (la résolvante r)
...
Nous allons maintenant exécuter le programme fact.pl afin de trouver la factorielle de 5 et stocker la réponse dans la variable F.
Le prompt Prolog est représenté par ``\verb+|?-+''\footnote{Le prompt Prolog ``|?-'' est une combinaison de |- (prouvable) et ?. Le prompt pose donc la question au système «est-ce qu'il existe une preuve».»} et la sortie standard par ``\verb+->+''.
...
\begin{equation}
\begin{array}{l}
(\forall l') (\forall l_1) (\forall l_2) (\forall l_3) (\forall x) \\
...
avec une clause du programme il faut {\em renommer} les variables de la clause car les variables sont quantifiées, l'universel étant éliminé.
